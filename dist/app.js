(() => {
  ...
  function renderToday() {
    ...
    app.innerHTML = `${pageHead(formatDate(selected, { weekday: "long" }), formatDate(selected, { month: "long", day: "numeric" }).toUpperCase())}
      <div class="date-strip">${dates.map(date => `<button class="date-pill ${iso(date) === iso(selected) ? "selected" : ""}" data-date="${iso(date)}"><span class="dow">${formatDate(date, { weekday: "narrow" })}</span><span class="num">${date.getDate()}</span><span class="mini-track"><i style="width:${percentFor(date)}%"></i></span></button>`).join("")}</div>
      ...
      <button class="sick-toggle ${state.sickDays.includes(iso(selected)) ? "active" : ""}" id="sickToggle">🤒 Feeling sick</button>`;
  ...
  function renderMonth() {
    ...
    app.innerHTML = `${pageHead(year, formatDate(monthCursor, { month: "long" }), actions)}<section class="stats-grid"><div class="card stat"><p class="card-kicker">Completions</p><strong>${stats.completions}</strong><small>items checked</small></div><div class="card stat"><p class="card-kicker">Active days</p><strong>${stats.activeDays}</strong><small>days with progress</small></div><div class="card stat"><p class="card-kicker">Perfect days</p><strong>${stats.perfectDays}</strong><small>every item met</small></div><div class="card stat"><p class="card-kicker">Most logged</p><strong>${stats.mostLogged}</strong><small>this month</small></div></section>
      <div class="month-layout"><section class="card month-card"><div class="weekday-row"><span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span><span>S</span></div><div class="month-grid">${cells.map(date => date ? `<button class="month-day ${iso(date) === iso(selected) ? "selected" : ""}" data-date="${iso(date)}" ${date > today ? "disabled" : ""}>${date.getDate()}<span class="day-progress"><i style="width:${percentFor(date)}%"></i></span></button>` : `<span class="month-day outside"></span>`).join("")}</div></section><aside class="card compact-card"><p class="card-kicker">${iso(selected) === todayIso ? "Today" : formatDate(selected, { weekday: "long" })}</p><h2>${formatDate(selected, { weekday: "long", month: "long", day: "numeric" })}</h2>${habitGroups(selected)}</aside></div>`;
